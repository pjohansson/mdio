...
impl RVec {
    ...
    pub fn from_fixed(input: &str, length: usize) -> Result<RVec, ParseRVecError> {
        ...
        let mut iter = input
            ...
            .map(|s| s?.trim().parse::<f64>().map_err(|_| ParseRVecError::ParseFloatError));
...
mod tests {
    ...
    fn add_rvec_operator() {
        let r1 = RVec { x: 0.0, y: 1.0, z: 2.0 };
        let r2 = RVec { x: 3.0, y: 4.0, z: 5.0 };
        assert_eq!(r1 + r2, RVec { x: 3.0, y: 5.0, z: 7.0 });

        let mut r3 = RVec { x: 0.0, y: 0.0, z: 0.0 };
        r3 += r1;
        assert_eq!(r3, r1);
    }
    ...
    fn sub_rvec_operator() {
        let r1 = RVec { x: 0.0, y: 1.0, z: 2.0 };
        let r2 = RVec { x: 3.0, y: 4.0, z: 5.0 };
        assert_eq!(r2 - r1, RVec { x: 3.0, y: 3.0, z: 3.0 });

        let mut r3 = r2.clone();
        r3 -= r2;
        assert_eq!(r3, RVec { x: 0.0, y: 0.0, z: 0.0 });
    }

    #[test]
    fn mul_rvec_with_usize() {
        let r = RVec { x: 1.0, y: 2.0, z: 3.0 };
        assert_eq!(r.pbc_multiply(2, 3, 4), RVec { x: 2.0, y: 6.0, z: 12.0 });
    }
    ...
    fn neg_rvec_operator() {
        let (x, y, z) = (1.0, 2.0, 3.0);
        let r = RVec { x, y, z };
        assert_eq!(-r, RVec { x: -x, y: -y, z: -z });
    }

    #[test]
    fn distance_between_rvecs() {
        let r1 = RVec { x: 1.0, y: 2.0, z: 3.0 };
        let r2 = RVec { x: 7.0, y: 11.0, z: 13.0 };
